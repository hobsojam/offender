...
static inline float triangleWave(float phase) {
...
struct SynthParams {
    ...
    float freqB      = 440.f;  // end frequency (exponential sweep if != freqA)
    float duration   = 0.2f;
    float volume     = 0.6f;
...
static Sound buildSound(const SynthParams& p) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        float freq;
        if (p.expSweep && p.freqA > 0.f && p.freqB > 0.f) {
            freq = p.freqA * powf(p.freqB / p.freqA, t);
        } else {
            freq = p.freqA + (p.freqB - p.freqA) * t;
        }
